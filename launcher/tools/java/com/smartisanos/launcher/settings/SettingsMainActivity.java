...
public class SettingsMainActivity extends BaseSettingsActivity {
    ...
    // A hosted second-level page remains inside this Activity.  The Host keeps
    // a one-level action for third-level pages; retain this root marker so a
    // second Back always returns to the landing page rather than Launcher.
    private boolean mHostedSecondLevelPage;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        bindHostedPage("setting_about_us", new Runnable() {
            @Override public void run() {
                MaintainedLauncherSettingsHost.showAboutPagePublic(SettingsMainActivity.this);
            }
        });

        // ── OCD 选项 / 附加功能 → AdditionalFeaturesActivity (阶段 2) ──────────
        bindHostedPage("setting_ocd_options", new Runnable() {
            @Override public void run() {
                MaintainedLauncherSettingsHost.showOcdOptionsPagePublic(SettingsMainActivity.this);
            }
        });
        ...
        bindHostedPage("item_id_icons", new Runnable() {
            @Override public void run() {
                MaintainedLauncherSettingsHost.showIconPagePublic(SettingsMainActivity.this);
            }
        });

        // ── 分身、隐私和动态图标保留原版内容/写入链，但交给真实 Activity 承接返回栈。 ──
        bindHostedPage("item_id_profile_apps", new Runnable() {
            @Override public void run() {
                MaintainedLauncherSettingsHost.showProfileAppsPageInSettingsPublic(SettingsMainActivity.this);
            }
        });
        ...
        bindHostedPage("item_id_privacy_password", new Runnable() {
            @Override public void run() {
                MaintainedLauncherSettingsHost.showPrivacyPasswordPageInSettingsPublic(SettingsMainActivity.this);
            }
        });
        bindHostedPage("item_page_flip_anims", new Runnable() {
            @Override public void run() {
                MaintainedLauncherSettingsHost.showPageFlipPagePublic(SettingsMainActivity.this);
            }
        });
        bindHostedPage("setting_dynamic_weather", new Runnable() {
            @Override public void run() {
                MaintainedLauncherSettingsHost.showDynamicWeatherPageInSettingsPublic(SettingsMainActivity.this);
            }
        });
    }
    ...
    public void onBackPressed() {
        if (MaintainedLauncherSettingsHost.handleModernSettingsBack(this)) {
            return;
        }
        if (mHostedSecondLevelPage) {
            mHostedSecondLevelPage = false;
            MaintainedLauncherSettingsHost.showMainSettingsPagePublic(this, true);
            return;
        }
        super.onBackPressed();
    }
    ...
    private void bindThemeWallpaperLaunch(String viewName, final String page) {
        ...
        if (row != null) {
            row.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    openHostedPage(new Runnable() {
                        @Override public void run() {
                            if (ThemeWallpaperActivity.PAGE_WALLPAPER.equals(page)) {
                                MaintainedLauncherSettingsHost.showWallpaperPagePublic(SettingsMainActivity.this);
                            } else {
                                MaintainedLauncherSettingsHost.showThemePagePublic(SettingsMainActivity.this);
                            }
                        }
                    });
                }
    ...
    private void bindHostedPage(String viewName, final Runnable page) {
        View row = findViewByName(viewName);
        if (row != null) {
            row.setOnClickListener(new View.OnClickListener() {
                @Override public void onClick(View v) {
    ...
    private void openHostedPage(Runnable page) {
        mHostedSecondLevelPage = true;
        MaintainedLauncherSettingsHost.setModernSettingsParentBackActionPublic(this,
                new Runnable() {
                    @Override public void run() {
                        mHostedSecondLevelPage = false;
                        MaintainedLauncherSettingsHost.showMainSettingsPagePublic(
                                SettingsMainActivity.this, true);
                    }
                });
        page.run();
    }
