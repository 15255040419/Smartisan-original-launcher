...
- **现代设置导航重构（当前实施中，OPPO 真机部分验证）**：已确认旧的 `SettingsMainActivity` 同时存在 Host `setContentView` 伪栈、`WeakHashMap<Activity, Runnable>` 返回动作和真实子 Activity 栈，导致系统边缘返回可直接结束设置任务。当前改动已将首页的关于、附加功能、主题/壁纸、应用图标、应用分身、隐私密码、动态天气和新增翻页动画入口改为显式的真实子 Activity；`BaseSettingsActivity` 的常规返回只 `finish()`，仅子页被异常单独启动时才补建首页。OPPO 真机已验证主题、翻页、隐私密码和应用分身的系统/边缘返回均回到原 `SettingsMainActivity`，不回桌面。主题详情、替换图标、密码验证后的管理页、城市选择等 L2/L3 页面仍有 Host 内页路径，尚未可以标记为全量完成；禁止以本条为依据恢复 `mHostedSecondLevelPage` 或首页 Runnable 伪栈。
...
- 【已废弃】桌面设置系统现代化（阶段 0 - 阶段 6）全量重构落地并通过构建与 Manifest 校验：
