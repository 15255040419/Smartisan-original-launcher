...
> 2026-07-22 导航实施更新：本文件此前的“仅审计、无代码实施”边界已不再适用于当前工作区。L0 到 L1 的真实 Activity 返回链正在实施：首页不再托管 L1 页面；已新增 `PageFlipSettingsActivity`；文本与二进制 Manifest 已纳入所有 L1。OPPO 真机已验证主题、翻页、隐私、分身的 L1 → L0 系统边缘返回。L2/L3（主题详情、替换图标、密码后续页、城市选择）仍未全部迁出 Host，状态为 `IMPLEMENTING`，不得把阶段 1–9 标为全量完成或 DEVICE_VERIFIED。
