...
> 实施状态（2026-07-22）：现代入口的 L0/L1 已开始改为真实 Activity 栈。`SettingsMainActivity` 不再保存 `mHostedSecondLevelPage` 或其父返回 Runnable；普通子页 Back 统一为 `finish()`，只在异常独立任务时恢复首页。legacy `ThemeChooserActivity → Host.show` 仍保留。Host 内的 L2/L3 页面尚待逐项迁出，因此本文档的目标结构并非全部已验收。
