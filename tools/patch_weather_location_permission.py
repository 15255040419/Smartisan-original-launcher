"""Add coarse location permission to the preserved binary AndroidManifest.xml."""
...
def main():
    ...
    if PERMISSION in strings:
        print("weather location permission already present")
        return
    ...
    permission = start_element(uses_permission_tag, [
        attribute(android_ns, name_attr, permission_value),
    ]) + end_element(uses_permission_tag)
    rebuilt[insert_at:insert_at] = permission
