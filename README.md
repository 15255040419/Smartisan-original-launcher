...
- 动态天气已恢复普通 Android 兼容：识别常见厂商系统天气入口，点击仍打开系统天气应用；桌面使用锤子原版天气、温度和昼夜动画，通过 Open-Meteo 获取天气，按一小时缓存刷新。“桌面设置 → 动态天气”可选择系统粗略定位或手动搜索城市，并可查看最近更新时间、立即刷新；手动城市模式不需要定位权限。Open-Meteo 非商业使用无需 API Key。
