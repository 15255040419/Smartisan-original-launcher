...
launcher APK size: about 54.8 MB
```
...
| 文件夹背景 | 普通主题文件夹背景当前优先从主 APK `assets/folder_theme_bg/` 读取与主题 ID 对应的背景图，避免 apktool 半反编译工程无法为新增 `wallpaper_preview_*` 自动补齐资源表导致回退默认黑底。透明主题仍保留原版模糊背景链路。 |
...
| 通知角标 | Android 6.0+ 使用系统通知监听服务把有效活动通知转换为原版 `new_message` 协议，沿用原版数字纹理、文件夹汇总和横扫清除动画；强迫症选项提供隐藏角标、横扫清除和通知使用权入口。横扫只忽略当时已有通知，新通知仍会重新显示角标。 |
...
- 文件夹背景防回归：当前主桌面工程不是完整 `aapt2` 资源工程，新增的 `wallpaper_preview_*` 不应再默认假设能进入主 APK 资源表。普通主题文件夹背景以 `assets/folder_theme_bg/` 为当前可信资产入口；若新增主题背景，优先补到这里，并沿用 `s.ug()` 的 assets 解码链路。
...
- 应用分身的历史实现记录存在阶段性方案切换；排查时请优先阅读 `DEVELOPMENT_LOG.md` 中 `2026-06-28` 与 `2026-06-29` 两节。旧的 `2026-06-06` 记录里关于 `PackageManager.getUserBadgedIcon` 的描述仅代表当时阶段，不再是当前可信基线。
- 天气保留了旧 Smartisan 资源与权限线索，但入口、刷新和普通 Android 兼容还没有完整回归；动态日历已完成普通 Android / VIVO Android 16 真机适配。
- 普通 Android 的角标以系统当前有效活动通知为数据源；应用未发送通知、关闭通知权限或没有提供真实未读数时，Launcher 无法读取应用私有未读数据库。
