...
最终目标不是做一个临时弹窗设置入口，也不是仿写一个“看起来像”的设置页。桌面设置必须优先迁移 `com.android.settings-100.apk` 中原生 Smartisan 桌面设置页面的结构、控件、资源和逻辑；`E:\FANG\smartisan\smartisan-launcher-maintained` 只作为普通 Android 兼容和缺失实现的参考。
...
`smartisan-launcher-maintained` 只能作为普通 Android 兼容、构建方式、部分设置逻辑的参考，不要直接切换到它作为主工程，也不要优先照着 maintained 重写设置界面。它原生只有 9 宫格 / 16 宫格，不满足本项目核心需求。

### maintained 使用原则

默认仍然以原版提取桌面和 `com.android.settings-100.apk` 的 Smartisan 设置为第一来源。但如果某个原生功能因为 Smartisan framework、系统私有服务、包名签名、package id 2 资源依赖等原因短期内无法可靠迁移，可以参考 `E:\FANG\smartisan\smartisan-launcher-maintained` 的独立桌面实现。

使用 maintained 时遵守三条规则：

1. 只借鉴可运行的普通 Android 兼容逻辑、权限跳转、默认桌面设置、存储 key、构建方式等实现。
2. 不直接替换当前工程，不把 12 / 20 宫格退回 maintained 的 9 / 16 宫格。
3. UI 优先保持 Smartisan 原生设置风格；如果必须用 maintained 的布局结构，也要在 README 记录原因，并尽量复用原生 Smartisan 的文字、顺序、图标、背景和交互。

### 备用主线：以 maintained 为基础移植 12 / 20 宫格

如果当前 `original-port` 继续被原生 Settings、二进制 Manifest、旧资源包和 Smartisan framework 依赖拖住，可以切换为“maintained 做主工程，original-port 提供 12 / 20 宫格和文件夹体验”的路线。

目标形态：

```text
maintained 的独立桌面能力
+ 当前 original-port 已验证的 12 / 20 宫格显示逻辑
+ 当前 original-port 已验证的文件夹打开态尺寸/资源
+ Smartisan 风格桌面设置入口和设置项
```

已确认 maintained 的基础：

- 最终 APK 约 41MB，明显比 original-port 当前约 102MB 更适合作为独立桌面基底。
- 已有普通 Android 兼容逻辑、独立桌面设置、默认桌面相关入口和构建流程。
- 已有 `FolderInfo`、`FolderCell`、`FolderPageView` 等文件夹相关类。
- assets 中已有部分 `12`、`folder/.../12`、`folder/.../20` 资源目录。
- 当前主要公开设置仍是 9 / 16 宫格，12 / 20 没有完整打通。

迁移顺序：

1. 不在 maintained 上直接大改。先复制出工作分支/目录，保留原 maintained 可构建状态。
2. 对比两个工程的 `Constants.smali`、`LayoutProperty.smali`、`assets/layout/**/MODE_*`，建立 9/16 与 12/20 的模式值、行列数、dock 坐标、folder 坐标映射表。
3. 把 original-port 中已经验证正常的 12 / 20 layout XML 和必要贴图迁入 maintained，只迁实际命中的 1080p / sw411dp / 通用资源，避免把 100MB 资源包整包搬过去。
4. 修改 maintained 的设置页：把 9 / 16 选项改为 12 / 20，写入 maintained 当前读取的 `launcher_mode` key。
5. 打通启动时读取模式、桌面重载、图标分页、拖拽落点、dock 进入编辑模式和文件夹创建/打开。
6. 验证 12 宫格：初始加载、图标位置、dock、编辑模式、拖拽、文件夹打开。
7. 验证 20 宫格：同上。
8. 再考虑把 original-port 的锤子原生视觉资源按需补进 maintained，而不是整包复制。

这条路线的判断：

- 用户补充：此前已经尝试过把 12 / 20 移植到 maintained，但桌面、编辑页和交互动画全部错位，最后放弃并转向当前 original-port。
- 因此这条路线不能再判断为“更简单”。12 / 20 和文件夹不是单纯资源替换，涉及布局、分页、拖拽落点、dock 动画和设置写入，风险明显高于只在当前 original-port 里补设置页。
- 当前优先级重新调整：保留 original-port 作为主线，因为它的 12 / 20 桌面、编辑模式、dock 交互和文件夹尺寸已经基本修正；设置页问题虽然难看，但边界更局部。
- maintained 只作为设置功能、普通 Android 兼容逻辑、默认桌面跳转和设置页排版参考，不再作为短期主工程迁移目标。
- 只有当 original-port 后续出现无法绕过的桌面主体问题，而不只是设置页问题时，才重新评估 maintained 主线。
...
- APK 应用名已从原版“桌面 / Launcher”改为“锤子桌面”；桌面内虚拟设置入口名称统一为“桌面设置”，并复用 maintained 的 `launcher_settings.png` 覆盖当前 `icon_setting` 资源作为入口图标。

## 当前设置页状态

设置页当前处于“临时承载页可打开，但方向需要纠正”的状态。
...
本节是后续实现的“产品规格”。先按这里确定功能、排列和交互，再逐项迁移代码。没有列在这里的功能，不要临时加。

#### 总体原则

- 页面外观、排列、交互优先对齐 `com.android.settings-100.apk` 的原生 Smartisan 设置。
- 当前桌面只保留和 launcher 强相关的功能；状态栏、电池、拨号盘等系统设置项不属于本桌面第一阶段目标。
- 每个功能先迁原生 UI 和原生交互，再接当前 launcher 能读取的 key。
- 如果原生功能依赖普通 Android 没有的系统服务，先禁用或隐藏，并在 README 记录原因；不要写一个“看起来差不多”的假功能。

#### 页面结构

一级入口：桌面图标“桌面设置” -> launcher 包内设置宿主。
...
主页面按原生截图和 `LauncherSettingsFragment` 顺序排列：
...
其中第 3、4 项也会出现在二级页面“主题、壁纸、图标”里。第一阶段可以先在主页面保留原生入口；二级页面后续再补齐。
...
- 拨号面板模式
- 隐藏唱机中轴
- 状态栏、电池、电量、充电剩余时间、网速等系统级选项
```

#### 功能清单

| 顺序 | 页面 | 功能 | 原生交互 | 当前 launcher 落地方式 | 第一阶段状态 |
| --- | --- | --- | --- | --- | --- |
| 1 | 主页面 | 默认桌面 | 点击进入默认 Home 选择页 `PreferredActivityChooserFragment` | 普通 Android 10+ 参考 maintained 请求 `ROLE_HOME`；不支持时跳 `android.settings.HOME_SETTINGS` / 默认应用设置 | 第一阶段已接入口 |
| 2 | 主页面 | 单板块视图预览 | `LauncherPreview` 展示当前模式预览，选择 12 / 20 宫格和材质 / 透明主题 | 迁 `LauncherPreview`、`PreviewSettingItemView`、`VerticalOptionsCheckView`；写 `launcher_mode`、`launcher_grid_theme` | 第一优先级 |
| 3 | 主页面 | 十二宫格 / 二十宫格 | 点击选项更新红勾、更新预览、保存默认多板块模式、更新 dock 预览 | 12 写当前桌面 12 模式，20 写当前桌面 20 模式；如果沿用原生 0x24/0x51，需要在适配层映射到当前 launcher 实际值 | 第一优先级 |
| 4 | 主页面 | 材质主题 / 透明主题 | 点击触发主题切换确认弹窗，确认后切换主题 | 写 `Settings.Global["launcher_grid_theme"]`，0=材质，1=透明；是否需要重启 launcher 要按原生逻辑验证 | 第一优先级 |
| 5 | 主页面 | 桌面主题 | 点击启动 `com.smartisanos.launcher.theme.ThemeChooserActivity`，带 `from_activity=launcher`、`from_settings=true` | 当前 `ThemeChooserActivity` 被临时占用为设置宿主，迁移时必须拆开：设置宿主独立，主题选择恢复给 ThemeChooserActivity | 第二优先级 |
| 6 | 主页面 | 桌面翻页动画 | 点击进入 `LauncherAnimPreviewFragment` | 迁原生动画选择页；写 `Settings.Global["launcher_page_animation"]`；当前 launcher 是否读取该 key 要逐项验证 | 第二优先级 |
| 7 | 主页面 | 桌面图标感知光影 | 原生 `SettingItemSwitch`，说明文字提示会增加耗电 | 需要确认当前 launcher 是否读取对应 key；未确认前先显示禁用或隐藏 | 第三优先级 |
| 8 | 主页面 | 隐藏桌面图标名称 | 原生 `SettingItemSwitch`，写 `launcher_hide_lable` | 写 `Settings.System["launcher_hide_lable"]`；当前读取位置在 `data/O.smali` | 第一优先级 |
| 9 | 主页面 | 解锁时桌面的动画效果 | 原生 `SettingItemSwitch`，写 `launcher_unlock_animation_enabled` | 需要确认当前 launcher 是否读取；未确认前先显示禁用或隐藏 | 第三优先级 |
| 10 | 主页面 | 单板块切换至多板块视图 | 原生底部说明卡，含左右滑动切换示意 | 功能与当前移植版实际多板块/单板块逻辑需要先确认；第一阶段只保留原生静态说明，不接业务 | 第三优先级 |
| 11 | 主题页 | 锁屏壁纸 | 进入壁纸选择 | 依赖系统壁纸/Smartisan 壁纸服务；第一阶段可隐藏 | 后续 |
| 12 | 主题页 | 桌面主题 | 进入主题选择 | 恢复 `ThemeChooserActivity` 原用途后接入 | 第二优先级 |
| 13 | 主题页 | 桌面翻页动画 | 进入动画选择 | 同主页面动画入口 | 第二优先级 |
| 14 | 主题页 | 应用图标 | 进入应用图标包选择 | 依赖主题/图标包服务；第一阶段可隐藏或禁用 | 后续 |
| 15 | 强迫症页 | 隐藏桌面图标名称 | 开关写 `launcher_hide_lable` | 同主页面隐藏名称；两个入口状态必须同步 | 第一优先级 |
| 16 | 强迫症页 | 隐藏图标上的角标 | 开关写 `launcher_hide_badge`；开启后隐藏“横扫清除角标”项 | 写 `Settings.System["launcher_hide_badge"]`；同步控制第 17 项可见性 | 第一优先级 |
| 17 | 强迫症页 | 紧贴屏幕横扫清除角标 | 开关写 `launcher_badge_swipe_clean` | 写 `Settings.System["launcher_badge_swipe_clean"]`；只有未隐藏角标时显示 | 第一优先级 |

#### 原生交互必须保持的点

- 12 / 20 宫格选项必须是原生红色圆形勾选态，不要用普通 RadioButton。
- 选择 12 / 20 后要立即更新预览和勾选态；是否需要重启 launcher，按原生确认弹窗逻辑决定。
- 材质 / 透明主题切换要走原生确认流程，不要无提示强杀桌面。
- 强迫症页里“隐藏图标上的角标”打开后，原生会隐藏“紧贴屏幕横扫清除角标”；这个显隐关系要保留。
- 主页面和强迫症页如果都有“隐藏桌面图标名称”，状态必须读取同一个 key 并保持同步。
- `ThemeChooserActivity` 不能同时承担“桌面设置宿主”和“桌面主题选择器”两个最终职责。迁移设置宿主后，主题选择要恢复到原生用途。

#### 一样一样实现顺序

按这个顺序做，每完成一项就在 README 的执行日志里记录：

1. 静态原生主页面骨架：标题、背景、卡片、列表项顺序必须和原生截图一致，不接功能。
2. 原生控件兼容层：`SettingItemText`、`SettingItemSwitch`、`SwitchEx`、`TitleBar`、`PreviewSettingItemView`。
3. 单板块视图卡片：`LauncherPreview` + 12 / 20 + 材质 / 透明的显示和选中态。
4. 12 / 20 宫格写入和桌面重载流程。
5. 隐藏桌面图标名称开关。
6. 强迫症页骨架。
7. 隐藏角标、横扫清除角标，以及二者显隐关系。
8. 桌面翻页动画二级页。
9. 桌面主题入口，恢复 `ThemeChooserActivity` 主题选择用途。
10. 透明主题、图标感知光影、解锁动画、应用图标、壁纸等重依赖功能。
